...
メニューバーまたは詳細設定の「タイマーを表示」を有効にすると、選択した出力先ディスプレイの上部中央へカウントダウンを最前面表示します。通常は小さな残り時間表示だけになり、ポインタを重ねたときだけパネルが拡大して操作ボタンを表示します。タイマーパネルはドラッグして好きな位置へ移動でき、その周囲のオーバーレイ領域はクリックを透過します。接続していない状態でも使用できます。
...
操作ボタンはタイマーパネルへポインタを重ねている間だけ表示されます。停止中に時間を調整すると、その値が次回以降の初期時間として保存されます。進行中または一時停止中の調整は、現在のカウントダウンだけに反映されます。5分未満になると、4分59秒から1分経過するごとに3秒間オレンジ色で強調します。0分00秒以降はポインタを外しても大きな赤色表示を続け、「停止」または時間追加で解除できます。
