...
メニューバーまたは詳細設定の「タイマーを表示」を有効にすると、選択した出力先ディスプレイの上部中央へカウントダウンを最前面表示します。タイマーパネルはドラッグして好きな位置へ移動でき、その周囲のオーバーレイ領域はクリックを透過します。接続していない状態でも使用できます。

- 「スタート」: 現在の残り時間からカウントダウンを開始・再開する
- 「一時停止」: 残り時間を保持したまま一時停止する
- 「停止」: カウントダウンを停止し、停止中に設定した初期時間へ戻す
- 「−1分」「−10秒」「＋10秒」「＋1分」: 残り時間を調整する

停止中に時間を調整すると、その値が次回以降の初期時間として保存されます。進行中または一時停止中の調整は、現在のカウントダウンだけに反映されます。5分未満になると、4分59秒から1分経過するごとに3秒間オレンジ色で強調します。0分00秒以降は赤色の強調表示を続け、「停止」または時間追加で解除できます。
