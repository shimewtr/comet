...
「参加用QRコードを表示」を有効にすると、選択中RoomのWebアプリURLをQRコードにして、出力先ディスプレイの右下へ表示します。「すべてのディスプレイ」では各画面に表示され、個別ディスプレイ選択時はその画面だけに表示されます。QRパネルはコメントと同じ最前面ポリシーとクリック透過を使うため、フルスクリーンスライドより前に表示しつつ操作を妨げません。
...
- スタンプの不透明度: 20〜100%
- 表示領域: 画面全体、上半分、上1/3
- 参加用QRコード: 選択中Roomの投稿URLを画面右下へ表示
