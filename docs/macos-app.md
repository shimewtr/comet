...
メニューバーまたは詳細設定の「タイマーを表示」を有効にすると、選択した出力先ディスプレイの右上へカウントダウンを最前面表示します。タイマーもクリックを透過するため、Keynote、Googleスライド、PowerPointなどの操作を妨げません。接続していない状態でも使用できます。
...
- 発表用タイマー: 画面右上へ表示し、開始、一時停止、停止、10秒・1分単位の調整が可能
