...
- macOSアプリから進行する、絵文字スタンプ投票と結果グラフ
- 任意で有効化できるOIDC認証
...
macOS 14以降では、メニューバー常駐アプリ「Comet」を使ってコメントとスタンプをディスプレイ全体へ表示できます。Chromeに限定されないため、KeynoteやPowerPointのフルスクリーンスライド、配信画面などにもクリック透過のオーバーレイを重ねられます。出力するディスプレイ、コメント速度、サイズ、不透明度、表示領域、参加用QRコード、発表用タイマー、絵文字スタンプ投票を設定できます。
...
1. メニューバーのCometアイコンを開き、WebアプリURLを入力する
2. 「接続する」を押す
3. Roomと出力先ディスプレイを選択する
4. 「オーバーレイを表示」をオンにする
5. 参加者へ投稿先を案内する場合は「参加用QRコードを表示」をオンにする
6. 発表時間を表示する場合は「タイマーを表示」をオンにし、画面上のタイマーへポインタを重ねて時間を調整してから再生ボタンを押す
7. 投票を行う場合は「投票を開始…」からタイトル・時間・ラベルを設定する。参加者は表示された `1️⃣`〜`8️⃣` のスタンプを送るだけで投票できる
